...
> "Is the WETH/USDC pool being drained right now?" → the agent resolves `api.rugradar.eth`, hits a `402`, signs a 0.02 HBAR payment, gets a risk score with evidence, and every payment plus the final decision lands on a Hedera Consensus Service topic you can audit on HashScan.
...
Prices are computed per request (x402 dynamic pricing), so a two-protocol scan and a six-protocol scan produce different `402` quotes.
...
# 5. the agent
pnpm agent "Is the WETH/USDC pool on Uniswap v3 being drained right now? What should an LP do?"
```
...
2. **Discovery (30 s).** `pnpm ens:resolve api.rugradar.eth` — show the records, the registry hierarchy (`ETHRegistry → our registry → our resolver`), expiry and `transferable: false`.
...
4. **Reason (60 s).** `pnpm agent "…"` — the agent discovers, reads the catalog, buys two or three calls within budget, streams the whale wire for 15 s, and prints a verdict with evidence and the list of payments. Show the `agent.decision` record on HCS.
5. **Standards leverage (30 s).** `pnpm graph:smoke` — the same query string running on six DEXes and three lenders; point at `packages/shared/src/graph.ts` and the one line that differs between schema 1.3 and 4.0.
...
- `packages/shared/src/protocols.ts` — nine protocols, one registry of subgraph IDs from Messari's deployment manifest.
- `packages/shared/src/graph.ts` — three query strings (`whales`, `pool-risk`, `lending-health`) serve nine protocols. The only divergence we hit between schema majors is the event actor field (`from` in 1.3, `account { id }` in 4.0), handled by a one-line switch. Adding a tenth protocol is one JSON entry.
...
| **Enhanced Access Control** delegation of exactly one text key to the service's key (`authorizeTextRoles`) | `register-service.ts` |
...
packages/ens       deploy-namespace, register-service, revoke-service, resolve, list (viem, Sepolia)
